...
def customer_identity_from_row(
    row: TupleRow, submitted_password: str
) -> PrincipalIdentity:
    if len(row) != len(CUSTOMER_COLUMNS):
    ...
    return PrincipalIdentity(
        id=principal_id,
        username=username,
        role=Role.from_customer(),
        principal_type=PrincipalType.from_customer(),
    )
